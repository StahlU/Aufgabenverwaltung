...
public class AufgabenGenerator {
    ...
    public static ObservableList<Mitarbeiter> generiereListe(int anzahl) {
        ...
        for (int i = 0; i < anzahl; i++) {
            ...
            int anzahlAufgaben = random.nextInt(5) + 1;
            for (int j = 0; j < anzahlAufgaben; j++) {
                String beschreibung = aufgaben[random.nextInt(aufgaben.length)];
                Aufgabe aufgabe = new Aufgabe(beschreibung);
                aufgabe.setErledigt(random.nextBoolean());
                mitarbeiter.getAufgaben().add(aufgabe);
            }
